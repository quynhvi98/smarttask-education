					...
					function() {
						...
						rows
								.each(function(index, row) {
									...
									$columnName
											.prepend(''
													+ '<span class="treegrid-indent" style="width:'
													+ 15 * level
													+ 'px"></span>' + '');
								});
						...
						reverseHide = function(table, element) {
							var $element = $(element), id = $element.data('id'), children = table
									.find('tr[data-parent="' + id + '"]');
							...
							if (children.length) {
								...
								$element.find('.treegrid-expander')
...
function treeTable2(tableClass) {
	$(tableClass).each(function() {
		var $table = $(this), rows = $table.find('tr');
		rows.each(function(index, row) {
			var $row = $(row), level = $row.data('level'), 
				id = $row.data('id'), 
